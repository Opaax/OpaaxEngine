# UI — record (branch `User_Interface`, 2026-09-14 → 2026-09-15) — CLOSED FOUR TIMES, user-verified

> **U13 closed 2026-09-15** on *"ok works, close the block"*. Their ask at the U11–U12 close (*"The
> preview: can be nice to zoom etc..."*): the designer's view — wheel zoom at the cursor, middle-drag
> pan, `F`/Fit, `1:1`, and the layout aspect as a named choice, since zoom decoupled the view from
> the layout. `feb96ce`. **921 / 10052 / 7.** Durable: UI14 amended. The world viewport's camera
> and gesture were reused whole; the renderer learned one optional view on a targeted submission.
> No correction came. **Named, not built:** snap-to-grid / alignment guides · a "show all outlines"
> toggle · rulers.
>
...
|---|---|---|
| **U13** | `feb96ce` | **The designer's view** (UI14 amended). `SubmitUICanvas(canvas, target, view*)`: a targeted submission may bring its own `CameraView`, and then the pass does not size the canvas. `Editor/UI/UIPreviewAspect.h` (Free / 16:9 / 21:9 / 4:3 / 16:10 / 9:16, exact ×120) — the LAYOUT TARGET, now that the view is not the whole canvas. The panel REUSES `EditorCamera` (+ a two-line `Set`) and `CameraGesture`: wheel zooms at the cursor, middle-drag pans (cursor wraps), `F`/Fit frames, `1:1` is the old picture; the pick, drag, grips and nudges map through the preview's view (`PreviewToCanvas` / `CanvasToPreview` / `UnitsPerPreviewPixel`); a grey frame marks the screen's edge; the view reseeds per document. 1 case, **920 → 921 / 10037 → 10052**. |
...
**Settled in U13, not the plan:** the plan held, and the one thing worth keeping is what made it
small: the world viewport's camera and gesture were already the right vocabulary (a position, an
ortho half-height, a middle-drag, a wheel at the cursor, a frame-to-fit), so the preview grew a
`Set` on the camera and nothing else of its own. The mapping change is the risk — every designer
gesture now goes through the preview's view instead of the canvas's — and no smoke reaches it:
**theirs, at 300 % and at 40 %.**
