...
namespace Opaax::Editor
{
    ...
    void AnimationLibraryPanel::DrawHeader(const AnimationLibraryData& InData)
    {
        ...
        // The default is what an animator naming no clip plays, so it is picked from the names that
    ...
    void AnimationLibraryPanel::DrawEntryList(const AnimationLibraryData& InData)
    {
        ...
        for (Uint32 lIndex = 0; lIndex < InData.EntryCount(); ++lIndex)
        {
            ...
            // The FIRST row is what an animator falls back to when the default names nothing, so it
            // is worth seeing which one that is without reading the combo.
    ...
    void AnimationLibraryPanel::DrawSelectedEntry(AnimationLibraryData& InData)
    {
        ...
        // CLOSE goes through LibraryOps, which is what judges the name against the rest of the list.
