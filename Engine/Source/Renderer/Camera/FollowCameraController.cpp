...
namespace Opaax
{
    ...
    void FollowCameraController::Tick(ICamera& InCamera, double InDeltaSeconds)
    {
        ...
        if (Maths::Abs(lDelta.x) <= m_Params.Deadzone.x &&
            Maths::Abs(lDelta.y) <= m_Params.Deadzone.y)
        {
            return;
        }

        // Snap when smoothing is effectively zero.
        if (m_Params.Smoothing <= FZERO)
        {
        ...
        const float    lAlpha  = FONE - Maths::Exp(-static_cast<float>(InDeltaSeconds) / m_Params.Smoothing);
