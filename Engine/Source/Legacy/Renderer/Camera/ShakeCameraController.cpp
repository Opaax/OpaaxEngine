...
namespace Opaax
{
    namespace
    {
        ...
        float DecayFactor(EShakeDecay InDecay, float InNormT)
        {
            ...
            if (InNormT >= FONE)
            {
                return (InDecay == EShakeDecay::Constant) ? FONE : FZERO;
            }
            ...
            switch (InDecay)
            {
                ...
                case EShakeDecay::EaseOut:  { const float k = FONE - InNormT; return k * k; }
                case EShakeDecay::Count:
    ...
    {
        if (m_Params.Duration <= FZERO || m_Params.Amplitude == FZERO)
        {
            // Mark expired immediately — controller will be pruned this frame.
            m_Elapsed = (m_Params.Duration > FZERO) ? m_Params.Duration : FONE;
            return;
        }
        ...
        const float lOmegaX = FTWO_PI * m_Params.Frequency;
        const float lOmegaY = lOmegaX * m_Params.YFrequencyRatio;

        const Vector2F lOffset{
            Maths::Sin(lOmegaX * m_Elapsed)                    * lAmp,
            Maths::Sin(lOmegaY * m_Elapsed + m_Params.YPhase)  * lAmp
        };
