...
TEST_CASE("GizmoDrag: a scale delta leaves the pivot itself untouched")
{
...
TEST_CASE("GizmoDrag: a scale delta scales OTHER points about the pivot, not the origin")
{
...
TEST_CASE("GizmoDrag: consecutive frames bank INCREMENTS, never the cumulative factor")
{
...
TEST_CASE("GizmoDrag: banked deltas COMPOSE while unspent")
{
...
TEST_CASE("GizmoDrag: a translate delta is a pure translation, wherever the pivot is")
{
    GizmoDrag lGizmo;
...
TEST_CASE("GizmoDrag: ConsumeDelta clears, so an unspent frame cannot be applied twice")
{
    GizmoDrag lGizmo;
...
TEST_CASE("GizmoDrag: a LOCAL scale of a rotated entity reads back clean in its own frame")
{
...
TEST_CASE("GizmoDrag: the pivot still holds still under a LOCAL scale")
{
    GizmoDrag    lGizmo;
...
TEST_CASE("GizmoDrag: a rotation delta is unaffected by which frame it is read in")
{
...
TEST_CASE("GizmoDrag: a scale reads clean in the GIZMO's frame, for every entity in the selection")
{
...
TEST_CASE("GizmoDrag: the frame survives a drag, because a drag never reseats")
{
...
TEST_CASE("GizmoDrag: a SHARED-pivot rotation orbits, which is what Individual must not do")
{
...
TEST_CASE("GizmoDrag: ReseatAt re-anchors BOTH matrices, so idling banks nothing")
{
