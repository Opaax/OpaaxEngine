...
|---|---|---|
| `Drawers()` | Inspector UI per game component, in **two forms** (2026-08-19, ARCHITECTURE.md **I15**): `Register<TComponent>()` is the DEFAULT drawer, folded from the fields the component itself declares with `OPAAX_PROPERTIES`; `Register<TComponent, TDrawer>()` is the hand-written override, for fields that need judgment. A component that is two floats costs two lines instead of a file — `HealthComponent` had no drawer at all and was invisible until it got one line. Which widget a field type gets is a `TPropertyDrawer<T>` **specialization**, not a registry entry, so a type nobody wrote one for is a compile error at the registration line rather than a blank row. | M2 |
| `ConfigDrawers()` | The same thing for a **config** (2026-08-20). It is not a second registry: `Drawers()` and `ConfigDrawers()` are two instantiations of one `TDrawerRegistry<TSubject>`, and the only difference is a `TDrawerResolver<TSubject, TTarget>` specialization answering what is drawable, how to resolve it, and whether the entry draws its own header. A config's fields come from the same `OPAAX_PROPERTIES` a component uses, nested groups included; a config nobody registers falls back to the Config panel's json view. | M2 |
...
| `ResourceTypes()` | browser CHROME for a resource type, keyed by **`ResourceTypeID`**: icon, optional label override, double-click (old `IAssetTypeActions` concept, rewritten injected). The extensions live on the type itself and are registered engine-side through `ModuleRegistrar::Resources()`. Named for the live vocabulary — `CResource`/`ResourceManager` — since `Asset` now means the retired `Legacy/Assets` world. | M2 |
...
`ResourceTypes()` registers **chrome only, keyed by resource TYPE** — an icon, an optional label override and a double-click. *This supersedes M2d's "the one route that takes a descriptor instead of type parameters", whose reasoning ("a file type is a string key plus two labels and a closure") held only while a type had ONE extension.* A texture is `.png` **and** `.jpg` **and** `.tga`: keyed per extension that is three entries repeating one icon and one closure, and adding `.webp` would mean editing the editor. So extension → type moved to the **engine** (`ResourceFormatRegistry`, an `EngineRegistries` member), where a type declares its own formats with `OPAAX_RESOURCE_FORMAT` and a game module registers it through `ModuleRegistrar::Resources()`. The editor now holds one entry per type and **names no extension anywhere**.

Two consequences worth stating. A game's file type is a **runtime** registration, not an editor one — `Sandbox.exe` knows `.wave` too, which is correct, since a wave is content. And normalization (lower-case, leading dot) had to move with the table: it is `NormalizeExtension` in the engine now, called by both the registry and the browser's scanner, because the two only agree byte-for-byte by calling the same function. `PathString::Extension` is its other half, in Core beside `Stem`. The browser's per-file lookup is still two integer compares.

The GUID-backed catalog (`.meta` sidecars, rename-safe references) belongs behind this same route when it lands — but it was **deliberately NOT built in M5** (user decision), because the reasoning that put it there does not hold. "Map files reference entities by Guid" is true and irrelevant: those are *entity* guids, they live in `EntityMeta`, and they already work. A *resource* guid would only matter once a component FIELD names a resource, and none does — `DummyComponent` is position/size/colour, `HealthComponent` is two ints, and textures are still deferred. Building it now would be an API whose only caller is its own test. **Trigger: the first component field that references a resource** (a sprite's texture).
...
**2026-08-19/20 — DONE, and it stayed small because it is a visitor, not a database.** `OPAAX_PROPERTIES` is a member-pointer list in Core (**I15**); the editor folds it. The `ConfigPanel` lists every registered config (`IConfigSystem::GetConfigs()`, walked **live** — `Get<T>()` auto-registers, so a config first read on any later frame simply appears) and draws the current one from its property list, falling back to `IConfig::ToText()` for a config nobody registered a drawer for. Scope, per the user: *"just a solution to avoid to create a drawer every time"* — the hand-written drawer stays as the override.

The three corrections that shaped it are worth keeping: a **type** says what a value is (`LinearColor`, not a "draw as colour" hint), a **facet** says how it behaves (`SetRange`, `SetFlags(NeedRestart)`), and **one registry** serves components and configs alike. Configs also stopped having a serialization idiom of their own (**BO1b**) — same macro as a component, ~220 lines of hand-written parsing deleted.
